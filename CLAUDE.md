...
- 구글 포토는 공식 API가 막혀 있으나 **링크 공유된 앨범은 무인증 접근 가능**하다.
  교수의 홈페이지용 상시 앨범 링크가 HANDOFF에 등록되면
  `python3 scripts/fetch-shared-album.py <링크> <저장폴더>`로 새 사진만 자동
  수집한다 (state 파일로 중복 방지). 앨범에 사진이 추가되면 같은 링크에서
  바로 보이므로, 사진 관련 작업 전에 이 앨범을 먼저 확인한다.
