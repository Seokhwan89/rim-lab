...
- 뉴스 이미지 표시 규칙 (2026-08-30): components/NewsImages.tsx 신설 —
  로드 후 naturalWidth/Height로 가로·세로를 판별해 클래스를 적용한다.
  단독 가로 max-w-2xl / 단독 세로(상장류) h-[340px] / 갤러리 가로
  h-40~52(cover) / 갤러리 세로 h-64~80(전체 표시). 상장 글씨가 읽히고
  단독 상장이 과대해지지 않는 것이 목적이므로 이 비율을 유지할 것.
- 로봇신문 항목: 대표 사진을 기사 본문의 교수 인터뷰 사진
  (2026-04-robot-news-2.jpg, cdn.irobotnews.com에서 수집)으로 교체,
  옛 사이트의 사이드바 캡처는 images[]로 유지 — 사진 2장+기사 링크 복원.
...
3-1. **NANO KOREA 사진**: 전시 부스·데모 사진이 최아리 8/3 메일 첨부와
   결과보고서에 있으나 Gmail 첨부 다운로드 도구가 없어 미수집 —
   교수가 올려주면 해당 뉴스 images[]에 추가.
