...
1. **특허 대표도 잔여분**: 준직구동 1건만 완료. 방법 확립됨 — Drive
   `download_file_content`는 대용량이면 결과가 로컬 파일로 떨어지므로
   base64 디코드 → pymupdf로 도면 페이지 렌더 → Fig.1 crop.
   후보 파일: P2025-0018US_출원서류.pdf(4-bar VTM), P2026-0085US.pdf,
   P2025-0114KR/0116KR 폴더. 등록특허(로봇용핸드 등)는 Drive에 특허증만
   있고 공보가 없어 교수가 명세서 PDF를 올려주면 처리. Google Patents
   직접 수집은 서버에서 503으로 막힘(단 사용자 브라우저용 링크는 정상).
   ※ Drive MCP 세션이 자주 만료됨 — 재연결 대기 후 재시도.
2. **뉴스 이미지 애매 보류분**: 시각 확인이 안 돼 안 붙인 것 —
   피자미팅 후보 news_all 77·79(강의실로 보임), ICROS 삼척 83·86,
   ICRA2025 043·046, 번개 그래픽(전기접착 과제 추정), MOTIE 후보 007-009.
   /news unmatched 10장(2021~2022 행사)은 news.ts 항목 자체가 없음 —
   항목 신설 여부는 교수 결정 (First robotic arm 2022.05, KSPE 2021
   수상 2022.02, 첫 논문 2022.01, Edgar 대학원연구상 2021.12, 지능형
   창작로봇 경연 2021.09)
