...
0. **구글 포토 사진 2장 대기** (교수 확인, 2026-08-30): ① TE608A(6층)
   ...
   **Google Takeout 예약 내보내기 → Drive 전달**: takeout.google.com에서
   Google 포토만 선택 → 연도 폴더 선택(예: 'Photos from 2026') → 전달 방법
   'Drive에 추가' → 빈도 '2개월마다(1년간)' 설정. 반출물이 Drive의 Takeout
   폴더에 zip으로 쌓이면 세션이 자동으로 발견·해제·색인해 사진 전체를 읽을
   수 있다 (Drive 검색어: title contains 'Takeout'). 현재 Drive에 Takeout
   폴더 없음 — 교수 설정 대기. 급한 사진은 채팅 업로드/공유 링크가 여전히
   가장 빠른 경로.
