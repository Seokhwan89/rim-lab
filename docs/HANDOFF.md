...
0. **구글 포토 사진 2장 대기** (교수 확인, 2026-08-30): ① TE608A(6층)
   ...
   **해결책 검증됨(2026-08-30): 구글 포토 공유 링크는 무인증 접근 가능.**
   교수가 사진 선택→공유→링크 복사해서 채팅에 붙여주면 끝. 수신 후:
   `curl -A "Mozilla/5.0..." <photos.app.goo.gl 링크>` → HTML에서
   `lh3.googleusercontent.com/pw/<id>` 추출 → `<url>=w1600`(또는 `=d`
   원본)으로 다운로드. 옛 공유 앨범으로 원본 화질까지 실증 완료.
