...
0. **TE608A(떼이야르관 6층) 연구실 사진**: 2026.07.01 TE208→608A 이전.
   교수 지시로 자동 확보 시도함(2026-08-30): Drive 최근 이미지 전수 검색
   (영수증·CES 자료뿐), Gmail 첨부 검색(행정 메일뿐) — 없음. 사진은 구글
   포토에만 있는데 구글 포토 API 커넥터가 없어 접근 불가. **교수가 Drive에
   올려주거나 메일로 보내주면** labGallery 'Workspace 3 (TE608A)'로 추가.
