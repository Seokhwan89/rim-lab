          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {projects.map((p, i) => (
              <Reveal key={p.slug} delay={(i % 3) * 80}>
                <Link href={`/research/${p.slug}`} className="card card-hover group flex h-full flex-col overflow-hidden p-0">
                  {p.image && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={p.image} alt={p.title} loading="lazy" className="h-44 w-full border-b border-rim-line bg-white object-contain p-2 transition-transform duration-300 group-hover:scale-[1.03]" />
                  )}
                  <div className="flex flex-1 flex-col p-7 pt-5">
                    <div className="flex items-center justify-between">
                      <span className="text-rim-cyan"><ProjectIcon kind={p.icon} className="h-8 w-8" /></span>
                      <span className="font-mono text-[11px] tracking-[0.2em] text-rim-faint">{String(i + 1).padStart(2, '0')}</span>
                    </div>
                    <h3 className="mt-4 font-display text-[18px] font-semibold leading-snug transition-colors group-hover:text-rim-cyan">{p.title}</h3>
